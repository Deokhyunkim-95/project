...
<c:forEach var="data" items="${showFoodShop}"> 
						<div class="restaurant_content">
							<section class="restaurant_content_detail">
								<h3><strong>${data.fname}</strong></h3>
								<p class="restaurant_content_style"> 음식 종류 : ${data.foodstyle} </p>
								<p class="restaurant_content_style"> 위치 : ${data.address} </p>
								<p class="restaurant_content_style"> 공휴일 : ${data.holiday} </p>
								<p class="restaurant_content_style"> 전화번호 : ${data.tel} </p>
							</section>
						</div>	
</c:forEach>							
							<!--리뷰리스트-->
...
<c:foreach var="data" items="${showFoodShop}">						
		...
		var options = { //지도를 생성할 때 필요한 기본 옵션
				center: new kakao.maps.LatLng(${data.latitude}, ${data.longtitude}), //지도의 중심좌표.
